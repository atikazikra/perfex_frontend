...
<div class="col-md-12">
    <div class="mb-2 mb-sm-4">
        <div class="_buttons">
            ...
            <a href="<?php echo admin_url('invoices/invoice'); ?>"
                class="btn btn-falcon-info pull-left new new-invoice-list me-1">
                <i class="fas fa-plus-circle me-1"></i>
                <?php echo _l('create_new_invoice'); ?>
            </a>
            <?php } ?>
            <?php if (!isset($project) && !isset($customer) && staff_can('create', 'payments')) { ?>
            <button id="add-batch-payment" onclick="add_batch_payment()" class="btn btn-falcon-info pull-left me-1">
                <i class="far fa-file-alt me-1"></i>
                <?php echo _l('batch_payments'); ?>
            </button>
            <?php } ?>
            <?php if (!isset($project)) { ?>
            <a href="<?php echo admin_url('invoices/recurring'); ?>" class="btn btn-falcon-info pull-left ">
                <i class="fas fa-retweet me-1"></i>
                <?php echo _l('invoices_list_recurring'); ?>
            </a>
            <?php } ?>
            <div class="display-block text-right">
                <div class="btn-group pull-right ms-1 invoice-view-buttons btn-with-tooltip-group _filter_data"
    ...
    if (total_rows(db_prefix() . 'invoicepaymentrecords', ['paymentmode' => $mode['id']]) == 0) {
        continue;
                ...
                <a href="#" class="btn btn-falcon-info btn-with-tooltip invoices-total"
                    onclick="slideToggle('#stats-top'); init_invoices_total(true); return false;" data-toggle="tooltip"
                    title="<?php echo _l('view_stats_tooltip'); ?>"><i class="far fa-chart-bar"></i></a>
            </div>
        </div>
    </div>
    <div class="row">
